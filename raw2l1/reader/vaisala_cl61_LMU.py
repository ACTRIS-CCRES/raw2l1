...
def init(data, dims, conf, logger):
    ...
    data["status_device_controller_temperature"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    data["status_device_controller_electronics"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    # data["status_device_controller_voltage"] = np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    data["status_device_controller_overall"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    data["status_optics_unit_accelerometer"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    ...
    data["status_receiver_solar_saturation"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    ...
    data["status_transmitter_light_source_power"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    data["status_transmitter_light_source_safety"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
    ...
    data["status_measurement_data_destination_not_set"] = (
        np.ones((dims["time"],), dtype=np.int16) * MISSING_INT
    )
...
def read_timedep_vars(data, nc_id, time_ind, logger):
    ...
    if data["float_fw_version"] >= 1.2:
        data["hkd_bkgd_radiance"][ind_b:ind_e] = nc_id["monitoring"].variables[
            "background_radiance"
        ][:]
        ...
        data["hkd_temp_trans"][ind_b:ind_e] = nc_id["monitoring"].variables[
        ...
        data["hkd_window_blower_heater"][ind_b:ind_e] = nc_id["monitoring"].variables[
    ...
    if data["float_fw_version"] == 1.1:
        data["status_inside_heater"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_inside_heater"][:]
        )
        data["status_window_blower_fan"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_window_blower_fan"][:]
        )
        data["status_window_blower_heater"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_window_blower_heater"][:]
        )
        ...
        # ][:])
        data["status_device_controller_temperature"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_device_controller_temperature"][:]
        )
        data["status_device_controller_electronics"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_device_controller_electronics"][:]
        )
        # data["status_device_controller_voltage"][ind_b:ind_e] = convert_str_to_int(nc_id.variables[
        #     "status_device_controller_voltage"
        # ][:])
        data["status_device_controller_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_device_controller_overall"][:]
        )
        data["status_optics_unit_accelerometer"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_optics_unit_accelerometer"][:]
        )
        data["status_optics_unit_electronics"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_optics_unit_electronics"][:]
        )
        ...
        # ][:])
        data["status_optics_unit_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_optics_unit_overall"][:]
        )
        data["status_optics_unit_memory"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_optics_unit_memory"][:]
        )
        data["status_optics_unit_tilt_angle"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_optics_unit_tilt_angle"][:]
        )
        data["status_receiver_electronics"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_receiver_electronics"][:]
        )
        data["status_receiver_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_receiver_overall"][:]
        )
        data["status_receiver_memory"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_receiver_memory"][:]
        )
        data["status_receiver_solar_saturation"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_receiver_solar_saturation"][:]
        )
        data["status_receiver_voltage"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_receiver_voltage"][:]
        )
        data["status_window_blocking"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_window_blocking"][:]
        )
        data["status_window_condition"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_window_condition"][:]
        )
        data["status_servo_drive_control"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_servo_drive_control"][:]
        )
        data["status_servo_drive_electronics"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_servo_drive_electronics"][:]
        )
        data["status_servo_drive_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_servo_drive_overall"][:]
        )
        data["status_servo_drive_memory"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_servo_drive_memory"][:]
        )
        data["status_servo_drive_ready"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_servo_drive_ready"][:]
        )
        data["status_transmitter_electronics"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_electronics"][:]
        )
        data["status_transmitter_light_source"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_light_source"][:]
        )
        data["status_transmitter_light_source_power"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_light_source_power"][:]
        )
        data["status_transmitter_light_source_safety"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_light_source_safety"][:]
        )
        data["status_transmitter_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_overall"][:]
        )
        data["status_transmitter_memory"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_transmitter_memory"][:]
        )
        ...
        data["status_maintenance_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_maintenance_overall"][:]
        )
        data["status_device_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_device_overall"][:]
        )
        data["status_recently_started"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_recently_started"][:]
        )
        data["status_measurement_status"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_measurement_status"][:]
        )
        data["status_datacom_overall"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_datacom_overall"][:]
        )
        data["status_data_generation_status"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_data_generation_status"][:]
        )
        # data["status_data_sending_status"][ind_b:ind_e] = convert_str_to_int(nc_id.variables["status_data_sending_status"][
        #     :
        # ])
        data["status_measurement_data_destination_not_set"][ind_b:ind_e] = convert_str_to_int(
            nc_id.variables["status_measurement_data_destination_not_set"][:]
        )

    if data["float_fw_version"] >= 1.2:
        data["status_inside_heater"][ind_b:ind_e] = nc_id["status"].variables["Inside_heater"][:]
        data["status_window_blower_fan"][ind_b:ind_e] = nc_id["status"].variables[
            "Window_blower_fan"
        ][:]
        ...
        data["status_optics_unit_overall"][ind_b:ind_e] = nc_id["status"].variables[
            "Optics_unit_overall"
        ][:]
        data["status_optics_unit_memory"][ind_b:ind_e] = nc_id["status"].variables[
            "Optics_unit_memory"
        ][:]
        ...
        data["status_receiver_overall"][ind_b:ind_e] = nc_id["status"].variables[
            "Receiver_overall"
        ][:]
        ...
        data["status_receiver_sensitivity"][ind_b:ind_e] = nc_id["status"].variables[
            "Receiver_sensitivity"
        ][:]
        data["status_receiver_voltage"][ind_b:ind_e] = nc_id["status"].variables[
            "Receiver_voltage"
        ][:]
        data["status_window_blocking"][ind_b:ind_e] = nc_id["status"].variables["Window_blocking"][
            :
        ]
        data["status_window_condition"][ind_b:ind_e] = nc_id["status"].variables[
            "Window_condition"
        ][:]
        data["status_servo_drive_control"][ind_b:ind_e] = nc_id["status"].variables[
            "Servo_drive_control"
        ][:]
        data["status_servo_drive_electronics"][ind_b:ind_e] = nc_id["status"].variables[
            "Servo_drive_electronics"
        ][:]
        data["status_servo_drive_overall"][ind_b:ind_e] = nc_id["status"].variables[
            "Servo_drive_overall"
        ][:]
        data["status_servo_drive_memory"][ind_b:ind_e] = nc_id["status"].variables[
            "Servo_drive_memory"
        ][:]
        data["status_servo_drive_ready"][ind_b:ind_e] = nc_id["status"].variables[
            "Servo_drive_ready"
        ][:]
        ...
        data["status_transmitter_overall"][ind_b:ind_e] = nc_id["status"].variables[
            "Transmitter_overall"
        ][:]
        data["status_transmitter_memory"][ind_b:ind_e] = nc_id["status"].variables[
            "Transmitter_memory"
        ][:]
        data["status_maintenance_overall"][ind_b:ind_e] = nc_id["status"].variables[
            "Maintenance_overall"
        ][:]
        data["status_device_overall"][ind_b:ind_e] = nc_id["status"].variables["Device_overall"][:]
        data["status_recently_started"][ind_b:ind_e] = nc_id["status"].variables[
            "Recently_started"
        ][:]
        data["status_measurement_status"][ind_b:ind_e] = nc_id["status"].variables[
            "Measurement_status"
        ][:]
        ...
        data["status_measurement_data_destination_not_set"][ind_b:ind_e] = nc_id[
            "status"
        ].variables["Measurement_data_destination_not_set"][:]
