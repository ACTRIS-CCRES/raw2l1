...
def create_2d_var(raw_data, data, var_names, conf, logger):
    ...
    for var_name in var_names:
        ...
        name = var_name[0]
        col_names = var_name[1]

        logger.debug('processing {} variables'.format(name))

        for col in col_names:

            # get columns which have the string in their name
            col_2_join = [col for col in column_names if col.startswith(name)]
