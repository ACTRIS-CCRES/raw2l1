...
ERR_HEX_MSG = [
    ...
    # bit 8
    {
        "hex": 0x00000100,
        "level": "ERROR",
        "msg": "Hardware failed or cable defect or absent",
        "fw": LAST_KNOW_FW,
    },
    # bit 9
    {
    ...
    # bit 13
    {
        "hex": 0x00002000,
        "level": "ERROR",
        "msg": "Laser disabled (related to temperature or laser safety)",
        "fw": LAST_KNOW_FW,
    },
    ...
    # bit 14
    {
        "hex": 0x00004000,
        "level": "ERROR",
        "msg": "Laser power or PIN signal too low or saturated",
        "fw": LAST_KNOW_FW,
    },
    # bit 15
    {
    ...
    # bit 22
    {"hex": 0x00400000, "level": "WARNING", "msg": "AFD problem", "fw": LAST_KNOW_FW},
    # bit 23
    {
        "hex": 0x00800000,
        "level": "WARNING",
    ...
    # bit 26
    {
        "hex": 0x04000000,
        "level": "WARNING",
        "msg": "Laser temperature out of range",
        "fw": LAST_KNOW_FW,
    },
    # bit 27
    {
        "hex": 0x08000000,
        "level": "STATUS",
        "msg": "High humidity inside the device",
        "fw": LAST_KNOW_FW,
    },
    # bit 28
    {
...
def init_data(vars_dim, conf, logger):
    ...
    data["humidity"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    ...
    data["temp_las"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    ...
    data["laser_power"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    ...
    data["p_ref"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["p_window"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["win_param"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["voltage_dc"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["voltage_det"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["voltage_las"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["voltage_led"] = np.ones((vars_dim["time"],), dtype=np.float32) * missing_float
    data["background_det"] = np.ones((vars_dim["time"],), dtype=np.int16) * missing_int
    data["fan"] = np.ones((vars_dim["time"],), dtype=np.int16) * missing_int
...
def read_timedep_vars(data, nc_id, soft_vers, time_ind, time_size, logger):
    ...
    logger.debug("reading window reflex")
    data["p_window"][ind_b:ind_e] = nc_id.variables["p_window"][:]
    if soft_vers >= 1.02:
        logger.debug("reading window reflex correction scale factor")
        data["win_param"][ind_b:ind_e] = nc_id.variables["win_param"][:]
    ...
    logger.debug("reading laser_power")
    data["laser_power"][ind_b:ind_e] = nc_id.variables["laser_power"][:]
    logger.debug("reading internal relative humidity")
    data["humidity"][ind_b:ind_e] = nc_id.variables["humidity"][:]

    logger.debug("reading dc voltage")
    data["voltage_dc"][ind_b:ind_e] = nc_id.variables["voltage_dc"][:]
    logger.debug("reading det voltage")
    data["voltage_det"][ind_b:ind_e] = nc_id.variables["voltage_det"][:]
    logger.debug("reading laser voltage")
    data["voltage_las"][ind_b:ind_e] = nc_id.variables["voltage_las"][:]
    logger.debug("reading led voltage")
    data["voltage_led"][ind_b:ind_e] = nc_id.variables["voltage_led"][:]
    logger.debug("reading detector background")
    data["background_det"][ind_b:ind_e] = nc_id.variables["background_det"][:]
    logger.debug("reading fan activity")
    data["fan"][ind_b:ind_e] = nc_id.variables["fan"][:]
    ...
    logger.debug("reading temp_las")
    data["temp_las"][ind_b:ind_e] = get_temp(nc_id.variables["temp_las"], logger)
    ...
    # -----------------------------------------------
    logger.debug("reading beta_att")
    data["rcs_0"][ind_b:ind_e, :] = nc_id.variables["beta_att"][:]

    logger.debug("reading p_calc")
    data["p_calc"][ind_b:ind_e] = nc_id.variables["p_cal"][:]
    data["meta"]["is_p_calc"] = True

    logger.debug("reading p_ref")
    data["p_ref"][ind_b:ind_e] = nc_id.variables["p_ref"][:]
    ...
    for ifile in list_files:
        ...
        if nb_files_read == 1:
            ...
            if overlap_file_info is not None:
                ...
                ovl_file_name = f"CHM8k_overlap_{ovl_file_date}.txt"
